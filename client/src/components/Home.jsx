...
export default function Home({ socket, playerName, setPlayerName, avatar, setAvatar, user, loginWithGoogle, logout, deleteAccount, activeTab, leaderboard, isGuest, setIsGuest, isAdmin, onOpenAdmin }) {
  ...
  const [lastSessionInfo, setLastSessionInfo] = useState(() => {
    try {
      const saved = localStorage.getItem('wana_last_session');
  ...
  // Keep last session code in sync with localStorage
  useEffect(() => {
    const checkLastSession = () => {
      try {
        const saved = localStorage.getItem('wana_last_session');
        setLastSessionInfo(saved ? JSON.parse(saved) : null);
      } catch {
        setLastSessionInfo(null);
      }
    };
    checkLastSession();
    window.addEventListener('storage', checkLastSession);
    return () => window.removeEventListener('storage', checkLastSession);
  }, []);

  // Listen to rejoin events to reset loading state
  useEffect(() => {
    if (!socket) return;
    const handleRejoinEnd = () => {
      setIsReconnecting(false);
    };
    socket.on('rejoin_failed', handleRejoinEnd);
    socket.on('rejoin_success', handleRejoinEnd);
    return () => {
      socket.off('rejoin_failed', handleRejoinEnd);
      socket.off('rejoin_success', handleRejoinEnd);
    };
  }, [socket]);
    ...
    if (!targetSessionId) {
      try {
        ...
        if (saved) {
          ...
          targetSessionId = parsed?.sessionId || null;
        }
      } catch {}
    }
      ...
      {activeTab === 'learn' && (
        <>
          <div className="card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', background: 'linear-gradient(to bottom right, var(--bg-surface), rgba(99, 102, 241, 0.1))' }}>
            ...
            {/* Bouton de reconnexion compact */}
            <div style={{ width: '100%', maxWidth: '300px', marginTop: '0.75rem' }}>
              <button
                type="button"
                ...
                style={{
                  ...
                  fontWeight: 600,
                  borderRadius: '12px',
                  background: isReconnecting ? 'rgba(99, 102, 241, 0.25)' : 'rgba(255, 255, 255, 0.06)',
                ...
                title="Vérifier et réintégrer votre dernière partie ou salle d'attente active"
              >
                <span style={{ fontSize: '1rem', display: 'inline-block', animation: isReconnecting ? 'spin 1s linear infinite' : 'none' }}>
                  {isReconnecting ? '⏳' : '🔄'}
                </span>
                <span>
                  {isReconnecting
                    ...
                    : lastSessionInfo?.sessionId
                      ? `Reconnexion (#${lastSessionInfo.sessionId})`
                      : 'Reconnexion rapide'}
                </span>
                {lastSessionInfo?.sessionId && !isReconnecting && (
                  <span style={{
                    fontSize: '0.65rem',
