...
export default function VengeanceMode({
  ...
  return (
    <div className={`vengeance-arena ${isShaking ? 'shake-effect' : ''}`}>
      ...
      >
        <div className={`vengeance-inner-content ${feedback?.type === 'success' || feedback?.type === 'purify' ? 'correct-flash' : ''} ${feedback?.type === 'wrong' || mustTypeCorrection ? 'wrong-flash' : ''} ${isExploding ? 'heart-icon-active' : ''}`} style={{ width: '100%', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%', overflow: 'hidden' }}>
          <BattleConsole
